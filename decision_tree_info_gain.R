...
#census.df <- census.df[(census.df$V2!="?" & census.df$V7!="?" & census.df$V14!="?"),]
...
census.processed <- data.frame("Age" = census.sample$V1,
                               ...
                               "Native_country" = census.sample$V14,
                               "Income" = census.sample$V15)
# fivenum(census.sample$V1)
# firstbin <- nrow(subset(census.sample,census.sample$V1>=87))
# census.cor <- cor(census.sample,use = "everything")

entropy <- function(k,k1,k2)
{
  res <- ((-k1/k)*log2(k1/k))+((-k2/k)*(log2(k2/k)))
  if(is.nan(res)) return(0) else return(res)
}

information.gain <- function(k,n,dist.values)
{
  #cat(k,n,dist.values)
  entropy.before <- entropy(k,dist.values[1],dist.values[2])
  #print(entropy.before)
  weighted.sum.of.entropies <- 0
  m <- 3
  while (n>0)
  {
    local.k <- dist.values[m]+dist.values[m+1]
    #print(local.k)
    weighted.sum.of.entropies <- weighted.sum.of.entropies + 
                                ((local.k/k)*(entropy(local.k,dist.values[m],dist.values[m+1])))
    #print(weighted.sum.of.entropies)
    n <- n - 1
    m <- m + 2
  }
  #print(weighted.sum.of.entropies)
  entropy.before - weighted.sum.of.entropies
}

#input.attributes <- attributes(census.processed)$names
...
get.info.gain.for.factor <- function(data.disc)
{
    ...
    ig <- information.gain(k.val,num.levels,values)
    ig
